...
{
    internal class LocalGrainDirectory : ILocalGrainDirectory, IDisposable
    {
        ...
        public void Start()
        {
            log.Info("Start");
            Running = true;
            this.runningTask = Task.Run(() => this.ProcessMembershipUpdates());
            if (maintainer != null)
            {
                maintainer.Start();
            }
        ...
        public async Task Stop(bool doOnStopHandoff)
        {
            ...
            if (this.runningTask != null) await this.runningTask;
        }
        ...
        internal async Task<AddressAndTag> RegisterAsync(ActivationAddress address, bool singleActivation, int hopCount, bool skipInitializationCheck)
        {
            ...
            var owner = await this.GetPartitionOwner(address.Grain, hopCount, callerMembershipVersion, "RegisterAsync", skipInitializationCheck);

            if (this.MyAddress.Equals(owner))
            {
            ...
            else
            {
                ...
                if (hopCount > 0)
                {
                    this.log.LogWarning($"RegisterAsync - It seems we are not the owner of activation {address}, trying to forward it to {owner} (hopCount={hopCount})");
                }

                // otherwise, notify the owner
                AddressAndTag result = await GetDirectoryReference(owner).RegisterAsync(address, singleActivation, hopCount + 1);
        ...
        public async Task UnregisterAsync(ActivationAddress address, UnregistrationCause cause, int hopCount)
        {
            ...
            var owner = await this.GetPartitionOwner(address.Grain, hopCount, callerMembershipVersion, "UnregisterAsync");
            ...
            else
            {
                if (hopCount > 0)
                {
                    this.log.LogWarning($"UnregisterAsync - It seems we are not the owner of activation {address}, trying to forward it to {owner} (hopCount={hopCount})");
                }

                UnregistrationsRemoteSent.Increment();
                // otherwise, notify the owner
                await GetDirectoryReference(owner).UnregisterAsync(address, cause, hopCount + 1);
            }
        }
            ...
            string context)
        {
            ...
            foreach (var address in addresses)
            {
                ...
                var owner = snapshot.CalculateGrainDirectoryPartition(address.Grain);

                if (owner != null && !owner.Equals(this.MyAddress))
                {
                    if (hopCount >= HOP_LIMIT)
                    {
                        // we are not forwarding because there were too many hops already
                        throw new OrleansException($"Silo {MyAddress} is not owner of {address.Grain}, cannot forward {context} to owner {owner} because hop limit is reached");
                    }
                    ...
                    if (!forward.TryGetValue(owner, out var list))
                    {
                        forward[owner] = list = new List<ActivationAddress>();
                    }
        ...
        public async Task<AddressesAndTag> LookupAsync(GrainId grainId, int hopCount = 0)
        {
            ...
            var owner = await this.GetPartitionOwner(grainId, hopCount, callerMembershipVersion, "LookUpAsync");
            ...
            else
            {
                if (hopCount > 0)
                {
                    this.log.LogWarning($"LookupAsync - It seems we are not the owner of grain {grainId}, trying to forward it to {owner} (hopCount={hopCount})");
                }

                // Just a optimization. Why sending a message to someone we know is not valid.
                if (!IsValidSilo(owner))
                {
                    throw new OrleansException(String.Format("Current directory at {0} is not stable to perform the lookup for grainId {1} (it maps to {2}, which is not a valid silo). Retry later.", MyAddress, grainId, owner));
                }

                RemoteLookupsSent.Increment();
                var result = await GetDirectoryReference(owner).LookupAsync(grainId, hopCount + 1);
        ...
        public async Task DeleteGrainAsync(GrainId grainId, int hopCount)
        {
            ...
            var owner = await this.GetPartitionOwner(grainId, hopCount, callerMembershipVersion, "DeleteGrainAsync");
            ...
            else
            {
                if (hopCount > 0)
                {
                    this.log.LogWarning($"DeleteGrainAsync - It seems we are not the owner of grain {grainId}, trying to forward it to {owner} (hopCount={hopCount})");
                }

                // otherwise, notify the owner
                DirectoryCache.Remove(grainId);
                await GetDirectoryReference(owner).DeleteGrainAsync(grainId, hopCount + 1);
            }
        }
